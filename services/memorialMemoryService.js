...
/** 上传回忆图片到 post-images（memorial-memories/ 前缀），返回 public URL */
export async function uploadMemoryImage(file, userId, petId) {
  ...
  const path = `memorial-memories/${userId || "anon"}/${petId}/${Date.now()}.jpg`;
...
/** 新增一条回忆卡片 */
export async function addMemory({ userId, petId, title, description, imageUrl, memoryDate, category }) {
...
export async function deleteMemory(memory) {
  ...
  try {
    ...
    if (idx >= 0) {
      ...
      if (path) await sb().storage.from("post-images").remove([path]).catch(() => {});
    }
  } catch { /* 删图失败不阻断删记录 */ }
