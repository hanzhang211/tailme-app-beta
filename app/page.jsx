...
══════════════════════════════════════════════════════════════ */
/* ── 多步骤新增宠物引导（手机号验证成功后进入）──────────────────
   7 步：①名字+性别 ②类型+品种 ③生日 ④体重 ⑤绝育+疫苗 ⑥性格(多选) ⑦AI专属形象
   仅做 UI / 交互升级；数据来源、提交逻辑、AI 生成逻辑全部沿用现有：
...
function Onboarding({ userId, onComplete }) {
  ...
  const back = () => { if (step > 1) { setShowGen(false); setStep((s) => s - 1); } };
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", height:36 }}>
          <button onClick={back} aria-label="返回"
            style={{ width:36, height:36, border:"none", background:"transparent",
                     cursor: step > 1 ? "pointer" : "default", fontSize:24, lineHeight:1,
                     color: step > 1 ? OB.text : "transparent", padding:0 }}>‹</button>
